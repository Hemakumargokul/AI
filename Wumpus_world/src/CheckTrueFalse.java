...
public class CheckTrueFalse {
	...
	public static void main(String[] args) {
			...
			while(  ( buffer = inputStream.readLine() ) != null) 
                        {
                                ...
                                {
                                	if(!buffer.contains("(")){
                                		model.put(buffer.toUpperCase(), true);
                                	...
                                	else
                                	{
                                		...
                                		String next=b.substring(2, b.length()-1);
                                		...
                                		if(!next.contains("(")){
                                			...
                                			if(sp[0].equalsIgnoreCase("NOT"))
                                			{
                                					model.put(sp[1].toUpperCase(), false);
		...
		knowledge_base.print_expression("\n");
		 symbols= knowledge_base.getSymbols(symbols);
		...
		symbols= statement.getSymbols(symbols);
		
		 HashSet<String> removeSymbols=(HashSet<String>)symbols.clone();
		 	for(String s2: removeSymbols)
		 	{
		 		if(model.containsKey(s2))
		 			symbols.remove(s2);
		 	}
		...
		for(Map.Entry<String, Boolean> m:model.entrySet())
			System.out.println(m.getKey()+" "+m.getValue());
		
		ArrayList<String> symbolsList=new ArrayList<String>(symbols);
		
		
		
		Boolean result1=TTEntail(knowledge_base,statement,symbolsList,model,0);
		Boolean result2=TTEntail(knowledge_base,statement,symbolsList,model,1);		
	 ...
	 * 
	 * notes: i'm not sure that I need the counterx
	 * 
	 */
	
	public  static Boolean TTEntail(LogicalExpression KB, LogicalExpression alpha,ArrayList<String> symbols,HashMap<String,Boolean> model, int statement_flag)
	{
		...
		if(symbols.isEmpty())
		{
			...
			for(Map.Entry<String, Boolean> m: model.entrySet())
				System.out.println(m.getKey()+"->"+m.getValue());
			//System.out.println(KB.PL_TRUE(model)+" "+model.get("P_4_4")+" "+model.get("B_4_4") );
			if(KB.PL_TRUE(model))
			{
				if(statement_flag==0)
				{
					System.out.println("alpha:"+alpha.PL_TRUE(model));
					return alpha.PL_TRUE(model);	
				}
				
				else
				{
					return (alpha.PL_TRUE(model)==true)?false:true;
				}
			}
			else
				return true;
		...
		//	System.out.println("Sym:"+sym);
			//symbols.remove(0);
			ArrayList<String> restSymbol=new ArrayList<String>();
			int i=0;
			...
			Boolean b1=TTEntail(KB,alpha,restSymbol,model,statement_flag);
			model.put(sym, false);
			Boolean b2=TTEntail(KB,alpha,restSymbol,model,statement_flag);
			System.out.println(b1+":dasd:"+b2);
			return b1 & b2;
		}
